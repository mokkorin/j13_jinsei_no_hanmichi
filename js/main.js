...
window.onload = function(){
	...
	var money_text = new Label('0');
	var intel_text = new Label();
	var turn_text = new Label(); // （樽）
	var job_text = new Label('0'); // (樽)
	...
	var GameScene = function(){
		...
		intel_text.text = (Players[myID-1].intelligent + '');
	    	if(turn % 7 == 0 && generation == ADULT && t_cnt == 0){
	    	    core.pushScene(PayDayScene(Players, 0));
	    	}
	    });
	...
	var PayDayScene = function(Players, t_cnt){
            var scene = new Scene();
		...
		if(flag == 0){
                    socket.emit('salary fin', myID, t_cnt);
