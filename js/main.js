...
window.onload = function(){
    ...
    core.onload = function(){
		...
		var GameScene = function(){
			...
			saikoro.ontouchstart = function(){
	    		if(isMyturn == 1){
					...
					/*テスト*/
					var job_list = new Array(1);
					job_list[0] = new Object();
					job_list[0].Name = "ニート";
					job_list[0].Salary = 0;
		...
		var jobSelect = function(job_list,Players,myID){
			var scene = new Scene();
			...
			for(var i = 0; i < job_list.length; i++){
				button[i] = new Sprite(400,40)
				button[i].image = core.assets['./image/job_button.png'];
				button[i].x = (WIDTH - 400) /2 ;
				button[i].y = 70 + (60 * i);
				scene.addChild(button[i]);
				
				job_text.moveTo(button[i].x + 5 , button[i].y + 5);
				if((Players[myID-1].intelligent) >= i*10){
					job_text.color = 'rgba(0, 0, 0, 1)';
				}else{
					job_text.color = 'rgba(0, 0, 0, 0.3)';
				}
				job_text.font = "25px Century";
				job_text.text = ("職業:" + job_list[0].Name + " " + 
								 "給料:" + (job_list[0].Salary + '') + " " +
